...
public class ReplyEnquiryForStaff implements ReplyEnquiry{
    /**
    ...
    public void replyEnquiry(User user) throws ModelNotFoundException, ModelAlreadyExistsException {
        Staff staff = StaffRepository.getInstance().getByID(user.getID());
        EnquiriesViewerForStaff enquiriesViewerForStaff = new EnquiriesViewerForStaff();
        List<Integer> optionsAvailable = enquiriesViewerForStaff.displayEnquiryInfo(staff);
        if(optionsAvailable.size()!=0){
            ...
            Enquiry e = new Enquiry(q.getCampName(),q.getQuestion(),reply,q.getAskedByStudentId(),"Staff",staff.getUserName());
            e.displayReplyInfo();
            if(!q.getAnswered()) {
